  ...
  return jest.fn((req, res, next) => {
    if (req.user.userType !== userType) {
      return res.status(403).json({
        success: false,
        message: `Access denied. Required user type: ${userType}`
      });
    }
    next();
  });
...
const requireProfileComplete = jest.fn((req, res, next) => {
  if (!req.user.profileCompletionStep || req.user.profileCompletionStep < 2) {
    return res.status(403).json({
      success: false,
